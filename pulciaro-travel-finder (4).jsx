...
export default function Pulciaro() {
  ...
  // Load saved tickets from persistent storage on mount
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("saved-tickets", false);
        if (res && res.value) setSavedTickets(JSON.parse(res.value));
      } catch (e) {
  ...
  const persistSaved = useCallback(async (list) => {
    try {
      await window.storage.set("saved-tickets", JSON.stringify(list), false);
    } catch (e) {
